...
public record Address
{
    ...
    {
        if (string.IsNullOrWhiteSpace(country) || country.Length > Constants.Constants.MIDDLE_TEXT_LENGTH)
            return Errors.General.LengthIsInvalid(nameof(country));
        if (string.IsNullOrWhiteSpace(city) || city.Length > Constants.Constants.MIDDLE_TEXT_LENGTH)
            return Errors.General.LengthIsInvalid(nameof(city));
        if (string.IsNullOrWhiteSpace(street) || street.Length > Constants.Constants.MIDDLE_TEXT_LENGTH)
            return Errors.General.LengthIsInvalid(nameof(street));
        if (string.IsNullOrWhiteSpace(house) || house.Length > Constants.Constants.MIDDLE_TEXT_LENGTH)
            return Errors.General.LengthIsInvalid(nameof(house));
        if (string.IsNullOrWhiteSpace(flat) || flat.Length > Constants.Constants.MIDDLE_TEXT_LENGTH)
